...
from .serializers import (BlogSerializer, UpdateBlogStatusSerializer, )
...
class BlogListView(ListAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        blog_data = Blog.object.filter(status=get_status)
        serializer = self.get_serializer(blog_data, many=True)

        for blog in serializer.data:
            get_user = User.object.filter(id=blog["user_id"]).values("first_name", "last_name", "email", "description",
...
class UpdateBlogStatusAPIView(UpdateAPIView):
    ...
    def get_queryset(self):
        ...
        return Blog.object.filter(id=blog_id)
...
class SelfBlogListView(ListAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        blog_data = Blog.object.filter(status="DREAFT"),filter(user_id=user_id)
        serializer  =self.get_serializer(blog_data,many=True)

        for blog in serializer.data:
            get_user = User.object.filter(id=blog["user_id"]).value("first_name", "last_name", "email", "description",
                                                                    "linkedin_url", "contact_number")
